...
if (typeof mermaid !== 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof mermaid !== 'undefined') {
      ...
      const mermaidBlocks = document.querySelectorAll('.wp-block-mermaid-blocks-diagram[data-mermaid]');
      mermaidBlocks.forEach(async (block, index) => {
        ...
        let renderContainer = block.querySelector('.mermaid-container');
        if (!renderContainer) {
        ...
        try {
          ...
          const {
            svg,
            bindFunctions
          } = await mermaid.render('render-' + index, code);
          ...
          renderContainer.innerHTML = '<p>Mermaid rendering failed. Check your syntax.</p>';
        }
      });
    } else {
      // eslint-disable-next-line no-console
      console.warn('Mermaid.js library is not available. Mermaid blocks will not be rendered.');
    }
  });
} else {
